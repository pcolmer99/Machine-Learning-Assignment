...
## Training Random Forest - No Additional Preprocessing - ~60 minutes to run
modfit <- train(classe ~., method="rf", data=xtraining)
## Training Random Forest - Preprocessing: Normalise skewed variables & remaining NAs - ~60 minutes to run
modfit2 <- train(classe ~., method="rf", preProcess=c("center", "scale", "knnImpute"), data=xtraining)
## Training Random Forest - Using K-Fold Cross Validation - ~5 minutes to run
## Not sure why it was so quick to run compared to the others - useds multiple threads maybe?
